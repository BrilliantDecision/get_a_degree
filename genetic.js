document.getElementById('start_button').addEventListener("click", start_algorithm);

// Main function algorithm
function start_algorithm() {
    pop_size = parseInt(document.getElementById('pop_count').value, 10);
    participants_count = parseInt(document.getElementById('participants_count').value, 10);
    mut_chance = parseInt(document.getElementById('mut_chance').value, 10);
    elite_percenage = parseInt(document.getElementById('elite_percenage').value, 10);
    it = parseInt(document.getElementById('it').value, 10);

    // Перекрашиваем эти рёбра
    for(let i = 0; i < all_edges.length; i++) {
        all_edges[i].color = '#11FFFF';
    }
    all_nodes = s.graph.nodes();
    n = all_nodes.length;
    matrix = create_matrix(n, all_nodes);
    population = create_population(n, pop_size);
    init_pop_size = population.length;
    current_it = 0;
    best_paths_mas = [];
    o = false;
    while(current_it < it) {
        pairs = tournament(participants_count, init_pop_size, population, matrix);
        new_pop = ordered_cross(pairs, population);
        neighbour_mutate(new_pop, mut_chance);
        elite_selection(elite_percenage, init_pop_size, new_pop, matrix);
        current_it += 1;
        mas = get_best_path(new_pop, new_pop.length);
        best_paths_mas.push([mas[0], population[mas[1]]]); // best length and path
        if(o == true) {
            // Перекрашиваем эти рёбра
            for(let i = 0; i < all_edges.length; i++) {
                all_edges[i].color = '#11FFFF';
            }
            path = population[mas[1]];
            redraw_path(path, edge_matrix);
        }
        else {
            o = true
            path = population[mas[1]];
            redraw_path(path, edge_matrix);
        }
        population = new_pop;
    }
}

// Create population
function create_population(n, pop_size) {
    population = [];
    one_population = [];
    for(let i = 0; i < n; i++) {
        one_population.push(i);
    }
    population.push(one_population);
    for(let i = 1; i < pop_size; i++) {
        new_arr = shuffle(one_population);
        population.push(new_arr);
    }
    return population;
}

// Get path length
function get_fitness(individual, matrix) {
    fitness_val = 0;
    for(let i = 0; i < individual.length - 1; i++) 
        fitness_val += matrix[individual[i]][individual[i + 1]]
    fitness_val += matrix[individual[individual.length - 1]][individual[0]]
    return fitness_val;
}

// Get length and index of the best path
function get_best_path(population, init_pop_size) {
    j = 0;
    best_len = get_fitness(population[0], matrix);
    for(let i = 1; i < init_pop_size; i++) {
        len = get_fitness(population[i], matrix);
        if(len < best_len) {
            best_len = len;
            j = i;
        }
    }
    return [best_len, j];
}

// Tournament
function tournament(participants_count, init_pop_size, population, matrix) {
    indexes = [];
    pairs = [];
    // Amount of tournaments
    for(let i = 0; i < init_pop_size; i++) {
        participants = [];
        for(let j = 0; j < participants_count; j++) {
            participant = getRndInteger(0, init_pop_size);
            participants.push([get_fitness(population[participant], matrix), participant])
        }
        // Соритруем участников по длине пути
        participants = participants.sort(function(a, b) {
            return a[0] == b[0] ? 0 : a[0] > b[0] ? 1 : -1;
        });
        indexes.push(participants[0][1]);
    }

    // Составляем пары
    rand_numbers = shuffle(indexes);
    for(let i = 0; i < init_pop_size; i++)
        if(indexes[i] != rand_numbers[i])
            pairs.push([indexes[i], rand_numbers[i]]);
    return pairs;
}

// Упорядоченный ОК создать особь
function ordered_cross_create_ind(first, second, population) {
    ////////////////////////////////////////////////////////////////////////////////
    couple = [];

    section = getRndInteger(1, population[0].length) // выбираем точку разреза
    c = population[first].slice(0, section);         // левая часть
    right = population[first].slice(section, population[0].length);      // правая часть

    indexes = []
    for(let i = 0; i < right.length; i++) {
        indexes.push([population[second].indexOf(right[i]), right[i]]);
    }
    sorted_i = indexes.sort(function(a, b) {
        return a[0] == b[0] ? 0 : a[0] > b[0] ? 1 : -1;
    });
    for(let i = 0; i < sorted_i.length; i++)
        c.push(sorted_i[i][1]);
    couple.push(c);
    ///////////////////////////////////////////////////////////////////////////////////////////
    c = population[first].slice(0, section);         // левая часть
    right = population[first].slice(section, population[0].length);      // правая часть

    indexes = []
    for(let i = 0; i < c.length; i++) {
        indexes.push([population[second].indexOf(c[i]), c[i]]);
    }
    sorted_i = indexes.sort(function(a, b) {
        return a[0] == b[0] ? 0 : a[0] > b[0] ? 1 : -1;
    });
    temp_arr = [];
    for(let i = 0; i < sorted_i.length; i++)
        temp_arr.push(sorted_i[i][1]);
    couple.push(temp_arr.concat(right));
    return couple;
}

// Упорядоченный оператор кроссинговера
function ordered_cross(pairs, population) {
    individuals = [];
    for(let i = 0; i < pairs.length; i++) {
        individuals = individuals.concat(ordered_cross_create_ind(pairs[i][0], pairs[i][1], population));
        individuals = individuals.concat(ordered_cross_create_ind(pairs[i][1], pairs[i][0], population));
    }
    return population.concat(individuals);
}

// Оператор случайной мутации - обмен местами двух соседей выбранного гена
function neighbour_mutate(population, mut_chance) {
    let temp;
    for(let i = 0; i < population.length; i++) {
        if(getRndInteger(0, 101) <= mut_chance) {
            g = getRndInteger(0, population[0].length)
            if (g == 0) {
                temp = population[i][population[0].length - 1];
                population[i][population[0].length - 1] = population[i][1];
                population[i][1] = temp;
            }
            else if (g == population[0].length - 1) {
                temp = population[i][g - 1];
                population[i][g - 1] = population[i][0];
                population[i][0] = temp;
            }
            else {
                temp = population[i][g - 1];
                population[i][g - 1] = population[i][g + 1];
                population[i][g + 1] = temp;
            }
        } 
    }
}

// Элитный отбор
function elite_selection(elite_percenage, init_pop_size, population, matrix) {
    new_pop = [];
    len_and_index = [];
    // Path len and index array
    for(let i = 0; i < population.length; i++) {
        len_and_index.push([get_fitness(population[i], matrix), i]);
    }
    // Sort that array
    len_and_index = len_and_index.sort(function(a, b) {
        return a[0] == b[0] ? 0 : a[0] > b[0] ? 1 : -1;
    });
    // Get indexes from sorted array
    vals = [];
    for(let i = 0; i < population.length; i++) {
        vals.push(len_and_index[i][1]);
    }
    // Get the elite
    elite_count = Math.trunc(init_pop_size * (elite_percenage / 100.0));
    for(let i = 0; i < elite_count; i++) {
        new_pop.push(population[vals[i]]);
    }
    j = elite_count;
    i = elite_count;
    while(i < init_pop_size && j < len_and_index.length - 1) {
        if(len_and_index[j][0] != len_and_index[j + 1][0]) {
            new_pop.push(population[vals[j]]);
            i ++;
        }
        j ++;
    }
    if(new_pop.length < init_pop_size) {
        for(let i = 0; i < init_pop_size - new_pop.length + 1; i++) {
            new_pop.push(population[vals[getRndInteger(elite_count, population.length)]]);
        }
    }
    return new_pop;
}
